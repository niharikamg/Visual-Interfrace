...
// Centering the map more accurately
const projection = d3.geoAlbersUsa()
  .scale(1000)  // Adjust scale if needed
  .translate([width / 2, height / 2 + 50]); // Shift downward slightly
...
const svg = d3.select(".viz").append("svg")
  .attr("width", width)
  .attr("height", height)
  .style("display", "block")  // Ensure block-level rendering
  .style("margin", "auto");   // Centering the SVG horizontally
...
const g = svg.append("g");
  ...
  function renderLegend(colorScale) {
    d3.select("#legend").remove();  // Remove old legend without affecting the map
    ...
    const legendSvg = d3.select("#legend-container")  
      ...
      .attr("height", legendHeight + 30); // Adjust for labels
